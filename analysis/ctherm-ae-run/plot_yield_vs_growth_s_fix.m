function plot_yield_vs_growth_s_fix(model_array, ko_array, prod_id, solution_id, varargin)
% NOTE: This fix simply turns off parfor, which is awfully slow
...
if strcmp(inputs.plot_type,'overlap')
    mut_colors = sns_colors('pastel');
    mut_line_colors = sns_colors('deep');
        ...
        for j =1:ndesigns
            for i = 1:nprod
                ...
                if j == 1 % title for the first row
                    title(inputs.prod_id{i},'FontSize',10,'FontWeight','Normal','Interpreter','none')
                end
