...
model = cb.io.load_json_model('./parent-model-generation/iML1515.json')
targets = pd.read_csv('./parse-targets/anaerobic_cands.csv')
...
with open ('pathway_table.csv', 'w') as f:
    ...
    for rxn_id in produce:
        ...
        met_id_no_comp = re.search('EX_(.*)_.*', rxn_id).group(1)
